...
import { Users, Plus, Trash2, Briefcase, Building2, ChevronDown, ChevronUp, Clock, Code } from "lucide-react";
...
function CandidateCardComponent({ candidate, picked, onToggle, all, weights }: { candidate: Candidate; picked?: boolean; onToggle?: (c: Candidate) => void; all: Candidate[]; weights: Weights }) {
  const { score, breakdown } = useMemo(() => computeCandidateScore(candidate, { weights, all }), [candidate, all, weights]);
  ...
  // Get matched skills for highlighting
  const matchedSkills = breakdown.matches || [];

  return (
    <div className={`rounded-2xl border-2 transition-all duration-200 hover:shadow-lg ${
      picked 
    ...
    } p-3 sm:p-4 shadow-sm flex flex-col`}>
      <div className="flex items-start gap-2 sm:gap-3">
        ...
        {onToggle && (
          <button 
            ...
            className={`inline-flex items-center gap-1 px-2 sm:px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 touch-manipulation ${
              picked 
            ...
            {skills.map((skill, index) => {
              ...
              return (
                <span
                  ...
                  title={isMatched ? `Matched skill: ${skill}` : skill}
                >
                  {skill}
                  {isMatched && (
      ...
      <details className="mt-3 sm:mt-4 text-sm">
        <summary className="cursor-pointer text-gray-700 font-medium hover:text-blue-600 transition-colors duration-200 flex items-center gap-2 touch-manipulation">
          <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
          <span className="text-xs sm:text-sm">Why they fit</span>
        </summary>
        <ul className="list-disc pl-4 sm:pl-5 mt-2 sm:mt-3 text-gray-600 space-y-1">
